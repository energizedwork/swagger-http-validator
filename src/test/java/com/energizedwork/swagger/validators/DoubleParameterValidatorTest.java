...
public class DoubleParameterValidatorTest {
    ...
    public void reports_error_when_value_is_not_a_number() {
        param = createParameterDef("number", "double");

        List<ValidationError> errors = validator(param).validateParam(param, "not_a_number");

        assertThat(errors.size(), is(1));
        assertThat(errors, hasItem(validationError("myParam", "input string \"not_a_number\" is not a valid double")));
    }
